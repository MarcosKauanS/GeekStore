...
import com.geekstore.geekstore.common.util.CartCookieUtil;
...
public class CartController {
    ...
    public String addToCart(@RequestParam("productId") Long productId,
        ...
        CartCookieUtil.saveCart(response, cart);
    ...
    public String viewCart(HttpServletRequest request,
        ...
        if (cart.isEmpty()) {
            cart = CartCookieUtil.getCart(request);
            saveCartToSession(session, cart);
        }
        ...
        try {
            ...
            for (CartItem item : cart) {
                Product product = productService.findById(item.getProductId())
                        .orElseThrow(() ->
                                new IllegalArgumentException("Produto inválido: " + item.getProductId()));

                Stock stock = stockService.findByProduct(product)
                        .orElseThrow(() ->
                                new IllegalArgumentException("Produto sem estoque: " + product.getName()));

                if (item.getQuantity() > stock.getQuantity()) {
                    throw new IllegalArgumentException(
                            "Quantidade solicitada maior que o estoque de: " + product.getName()
                    );
                }
            ...
            cart.clear();
            saveCartToSession(session, cart);
            CartCookieUtil.clear(response);

            return "redirect:/orders/confirmation/" + order.getId();

        } catch (IllegalArgumentException e) {
            model.addAttribute("errorMessage", e.getMessage());
            return viewCart(request, model, userDetails);
        }
    }
}
